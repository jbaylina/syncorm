		...
		U.each(self.$schema.reverseRelations, function(reverseRelation, f) {
			if (reverseRelation.visibility.indexOf(selector) >= 0) {
				...
				mk.each(self[f], reverseRelation.cls.prototype.$schema.id.length, function(relobj, key) {
					obj[f].push(relobj.toJSON(selector));
				});
			}
		});
		return obj;
...
/*					U.each(rows, function(row) {
		...
		function extratLastDbOpId(cb) {
			...
			self.$driver.pool.query("SELECT MAX(id) as maxId from dbops", function(err, rows) {
				if (err) {
					self.$log('warning', 'Error in select max dbops: '+ err.toString());
					return cb(err);
				}
			...
			self.$driver.startTransaction(function(err) {
				...
				self.refreshDatabase(true, function(err) {
					...
					try {
						...
						self.$log("warning", err2.toString());
...
//								console.log("row: " + JSON.stringify(row) +  "  result: " + JSON.stringify(rows2));
					...
					U.each(rowsss, function(row) {
						if (row.clsName) {
							...
							if (row.op === "INSERT") {
								...
								} else if ((row.obj)&&(!mk.get(self[sch.table],row.key))) {
									var tmp2 = new self[sch.name](row.obj);
								}
                    ...
                    var lastDeleteDay = Math.floor((self.$timeLastDeleteOldOps - 4*3600*1000) / 86400000);
                    var today = Math.floor((new Date().getTime()-(4*3600*1000)) / 86400000);

                    if ((self.$deleteOldDbOps) && (deleteOldDbOps) && (today>lastDeleteDay)) {
                        self.$timeLastDeleteOldOps = (new Date().getTime());
                        var lastOp = self.lastDbOpsId;
                        U.defer(function () {
                            self.$driver.deleteOldDbOps(lastOp, function (err) {
                                if (err) {
                                    self.$log("warning", "Error deleting dbOps: " + err);
                                } else {
                                    self.$log("info", "deleting old dbOps Ok " + ((new Date().getTime()-self.$timeLastDeleteOldOps)/1000) + " seconds!");
                                }
                            });
                        });
                    }

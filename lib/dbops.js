...
function cloneNorm(obj) {
	...
	var fieldName, temp = {};
	for (fieldName in obj.$schema.fields) {
		if (obj.$schema.fields.hasOwnProperty(fieldName)) {
		...
		function execOp(q, aop) {
			return q.then(function () {
				...
				if (aop.action === "insert") {
					...
					return db.$driver.update(aop.obj.$schema.table, aop.newValues, aop.obj);
				} else if (aop.action === "delete") {
					return db.$driver.delete(aop.obj.$schema.table, aop.obj);
				}
			});
		}

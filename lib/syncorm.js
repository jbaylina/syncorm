...
function createFieldProperty(db, cls, f, db2js, js2db) {
	...
	Object.defineProperty(cls.prototype, f, {
		get: function () {
			var val=  db2js(this.$data[dbFieldName], sch.fields[f]);
			if (typeof sch.fields[f].getter === "function" ) {
				val = sch.fields[f].getter.call(this, val, f);
			}
			return val;
		},
		set: function (val) {
...
/*					U.each(rows, function(row) {
					...
					async.eachLimit(rows, 500, function(row, cb) {
						if (i%1000 === 0)
							console.log(Cls.prototype.$schema.name+ " "+i);
						i+=1;
						var o = new Cls("loaddirect", row);
						mk.set(Cls.prototype.$all,o.$key,o);
						U.defer(cb);
		...
		self.$driver.getAll('dbops',
			...
			function(err, rowsss) {
				...
				async.eachSeries(rowsss, function(row, cb2) {
					...
					if ((self.$deleteOldDbOps)&&(deleteOldDbOps)) {
						self.$driver.deleteOldDbOps(self.lastDbOpsId, function(err) {
							cb(err);
						});
					} else {
						U.defer(cb);

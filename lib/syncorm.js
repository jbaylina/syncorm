...
exports.Database = function(params) {
	...
	this.$removeFromIndexes = function(obj) {
		U.each(this.$indexes, function(index, indexName) {
			if (index.className === obj.$schema.name) {
				if (index.indexFunction(obj)) {
					self[indexName].remove(obj);
				}
			}
		});
	};
	...
	this.define = function (sch) {
		...
		cls = function (a, b) {
			...
			if (self2.$all[self2.$key]) {
				throw new Error("This object already exist: Schema: " + self2.$schema.name + " Key: "+self2.$key);
			}

			self2.$all[self2.$key] = self2;
			...
			self.$driver.getAll(
				...
				function(err, rows) {
					...
					U.each(rows, function(row) {
						...
						Cls.prototype.$all[o.$key] = o;
					});
					cb();
				});
		}

		function construnctIndex(indexName) {
			...
			self[indexName] = new RBTree(function (a,b) {
				if (typeof a === "object") {
					a = index.indexFunction(a);
				}
				if (typeof b === "object") {
					b = index.indexFunction(b);
				}
		...
		U.each(obj.$schema.relations, function(rel, relName) {
			...
			if (toObj) {
				...
				if (rel.reverse) {
					...
					toObj.$reverseRelations[rel.reverse][obj.$key] = obj;
				}
			} else {
				if (rel.reverse) {
					if (!obj.$orphans[relName][k]) obj.$orphans[relName][k] = {};
					obj.$orphans[relName][k][obj.$key] = obj;
				}
			}
		});
		U.each(obj.$schema.reverseRelations, function(revRel, revRelName) {
			var orphans = revRel.cls.prototype.$orphans[revRel.relation][obj.$key];
			if (orphans) {
				U.each(orphans, function(orphan) {
					obj.$reverseRelations[revRelName][orphan.$key] = orphan;
					orphan.$relations[revRel.relation] = obj;
					delete revRel.cls.prototype.$orphans[obj.$key];
				});
			}
		});
	};
	...
	this.$destroyRelations = function(obj) {
		...
		U.each(obj.$schema.reverseRelations, function(revRel, revRelName) {
			U.each(obj.$reverseRelations[revRelName], function(fromObj) {
				if (!fromObj.$orphans[revRel.relation][obj.$key]) {
					fromObj.$orphans[revRel.relation][obj.$key] = {};
				}
				fromObj.$orphans[revRel.relation][obj.$key][fromObj.$key] = fromObj;
				delete fromObj.$relations[revRel.relation][obj.$key];
			});
		});
		U.each(obj.$schema.relations, function(rel, relName) {
			var toObj = obj.$relations[relName];
			if ((toObj)&&(rel.reverse)) {
				delete toObj.$reverseRelations[rel.reverse][obj.$key];
			}
		});
		U.each(obj.$schema.reverseRelations, function(revRel, revRelName) {
	...
	this.getClassNameFromTableName = function(dbTableName) {
		...
		self.$driver.getAll('dbops',
			...
			function(err, rows) {
				...
				async.each(rows, function(row, cb2) {
					...
					row.clsName = self.getClassNameFromTableName(row.table);

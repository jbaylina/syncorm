		...
		for (field in sch.fields) {
			if (sch.fields.hasOwnProperty(field)) {
				...
				if (!sch.fields[field].type) {
					...
					sch.fields[field].$type = types[sch.fields[field].type];
				} else {
					...
					sch.fields[field].$type = types.link;
				}
			}
		}
	...
	this.loadAll = function (onInit) {
					...
					if (objFrom[field]) {
						if (To.prototype.$all[objFrom[field]] === undefined) {
							...
							if (reverse) {
								...
								objTo.$associations[reverse][objFrom.$key] = objFrom;
							}
						}
					}
			});
		...
		function loadAllObjects(q, Cls, connection) {
			...
			return q.then(function () {
				...
				self.$driver.getAll(Cls.prototype.$schema.table,

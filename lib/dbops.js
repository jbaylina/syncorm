...
function js2db(schema, obj) {

	var temp = {};

	U.each(obj, function(val, fieldName) {
		if (schema.fields[fieldName]) {
			temp[schema.fields[fieldName].dbFieldName] = val;
		}
	});
...
var DbOp = function (action, obj, fieldName, oldValue, newValue) {
	...
	if (action === "insert") {
		this.oldValues = null;
		this.newValues = orm2js(obj);
	} else if (action === "delete") {
		this.oldValues = orm2js(obj);
		this.newValues = null;
		...
		this.oldValues[fieldName] = oldValue;
		...
		async.eachSeries(Object.keys(self.ops), function(aopk, cb2) {
			...
			if (!aop) {
				...
				if (aop.obj.$schema.fields[aop.obj.$schema.id[0]].autoincrement) {
					...
					oldKey = aop.obj.$key;
					db.$driver.insert(aop.obj.$schema.dbTableName, js2db(aop.obj.$schema, aop.newValues), function(err, result) {
					...
					db.$driver.insert(aop.obj.$schema.dbTableName, js2db(aop.obj.$schema, aop.newValues), cb2);
				}
			} else if (aop.action === "update") {
				db.$driver.update(aop.obj.$schema.dbTableName, js2db(aop.obj.$schema, aop.newValues), aop.obj, cb2);

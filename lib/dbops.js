...
//	return field.$type.db2js( field.$type.js2db ( value, field), field);
	return value;
}
...
function cloneNorm(obj) {
	...
	U.each(obj.$schema.fields, function(field, fieldName) {
		var S = normalize(field,obj[fieldName]);
...
var DbOp = function (action, obj, fieldName, oldValue, newValue) {
	...
	if (action === "insert") {
		...
		oldValue = normalize(field,oldValue);
		newValue = normalize(field,newValue);

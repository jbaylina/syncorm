...
function js2db(obj) {
	...
	U.each(schema.fields, function(val, fieldName) {
		var v = schema.fields[fieldName].$type.js2db(obj[fieldName], schema.fields[fieldName]);
		if (v !== null) {
...
var DbOp = function (action, obj, fieldName, oldValue, newValue) {
	...
	if (action === "insert") {
		...
		this.oldValues[obj.$schema.fields[fieldName].dbFieldName] = obj.$schema.fields[fieldName].$type.js2db(oldValue, obj.$schema.fields[fieldName]);
		this.newValues[obj.$schema.fields[fieldName].dbFieldName] = obj.$schema.fields[fieldName].$type.js2db(newValue, obj.$schema.fields[fieldName]);
	} else {
...
*/
	this.rollback = function (cb) {
		...
		U.each(this.ops, function(op) {
			if (op) {
				err = new Error("Rollback faled: incosistency");
				var S = JSON.stringify(op);
				var SO = JSON.parse(S);
				db.$log("error", err.toString(), SO);
			}
		});
